...
type SessionLike = {
	...
	command: {
		executeCommand(
			command: string,
			timeoutMs?: number,
		): Promise<Operation & { output: string; exitCode?: number }>;
	};
	fileSystem: {
	...
	getLink?(
		protocolType?: string,
		port?: number,
	): Promise<Operation & { data?: unknown }>;
	browser?: {
...
export function safeMessage(error: unknown): string {
	...
	return message
		...
		.replace(
			/\b(?:s-[a-z0-9]+|link-\d+-\d+|[0-9a-f]{8}(?:-[0-9a-f]{4}){3}-[0-9a-f]{12})\b/giu,
			"<redacted-id>",
		)
		.replace(/\btenantId\s*[:=]\s*[A-Za-z0-9-]+/giu, "tenantId: <redacted>")
		.replace(
			/\b(accesskeyid|accesskey|securitytoken|signature)\s*[:=]\s*[^&\s]+/giu,
			"$1=<redacted>",
		)
...
function requireApiKey(): void {
	if (!process.env.AGENTBAY_API_KEY?.trim()) {
		throw new Error(
			"缺少 AGENTBAY_API_KEY；请写入 ~/.config/agentbay/api_key 后重试",
		);
	}
}
...
async function createRemoteSandbox(
	...
	const idleMinutes = Math.max(
		3,
		Math.ceil(Math.max(idleMs, previewTtlMs) / 60_000),
	);
	const result = await new AgentBay().create({
		...
		lifecyclePolicy: new LifecyclePolicy({
			idleReleaseTimeout: idleMinutes,
			maxRuntime: 120,
		}),
	});
	if (!result.success || !result.session) {
		throw new Error(
			`AgentBay 沙盒创建失败：${result.errorMessage || "未知错误"}`,
		);
	}
...
function validHttpUrl(value: unknown, label: string): string {
	...
	if (!url) throw new Error(`${label}未返回有效 URL`);
	if (url.protocol !== "https:" && url.protocol !== "http:")
		throw new Error(`${label}协议无效`);
	url.search = url.search.replace(/\+/gu, "%2B");
	return url.toString();
}

function destroyingDetail(
	reason: "idle" | "closed" | "reset" | "previewExpired",
): string {
	switch (reason) {
		case "idle":
			return "10 分钟无活动，正在销毁沙盒…";
		case "previewExpired":
			return "临时预览已到期，正在销毁沙盒…";
		default:
			return "正在销毁沙盒…";
	}
}

function destroyedDetail(
	reason: "idle" | "closed" | "reset" | "previewExpired",
): string {
	switch (reason) {
		case "idle":
			return "沙盒已因空闲销毁；输入 /new 继续";
		case "previewExpired":
			return "临时预览已到期并销毁；输入 /new 继续";
		default:
			return "沙盒已销毁";
	}
}

// 沙盒适配器集中管理生命周期、预览和远程 I/O 状态。
// pi-lens-ignore: ast-grep:large-class
export class EphemeralSandbox implements PreviewRuntimePort {
	...
	constructor(
		...
		this.idleMs =
			options.idleMs ?? Number(process.env.AGENTBAY_IDLE_MS || DEFAULT_IDLE_MS);
		this.previewTtlMs =
			options.previewTtlMs ??
			Number(process.env.AGENTBAY_PREVIEW_TTL_MS || DEFAULT_PREVIEW_TTL_MS);
		if (!Number.isFinite(this.idleMs) || this.idleMs <= 0)
			throw new Error("AGENTBAY_IDLE_MS 必须是正数");
		if (!Number.isFinite(this.previewTtlMs) || this.previewTtlMs <= 0)
			throw new Error("AGENTBAY_PREVIEW_TTL_MS 必须是正数");
		this.createSandbox =
			options.createSandbox ??
			(() => createRemoteSandbox(this.idleMs, this.previewTtlMs));
	...
	async start(): Promise<void> {
		...
		this.startPromise = (async () => {
			...
			try {
				...
				this.setStatus(
					"ready",
					`AgentBay 已就绪 · ${Math.round(this.idleMs / 60_000)} 分钟无活动自动销毁`,
				);
				this.touch();
			} catch (error) {
	...
	async execute(command: string, timeoutMs = 60_000): Promise<string> {
		...
		const output = clipped(
			result.output || result.errorMessage || "（无输出）",
		);
		if (!result.success)
			throw new Error(
				`远程命令失败（exit ${result.exitCode ?? "?"}）：${safeMessage(output)}`,
			);
		return output;
	}
	...
	async readFile(path: string): Promise<string> {
		...
		if (!result.success)
			throw new Error(
				`远程文件读取失败：${safeMessage(result.errorMessage || path)}`,
			);
	...
	async writeFile(path: string, content: string): Promise<string> {
		...
		if (!result.success)
			throw new Error(
				`远程文件写入失败：${safeMessage(result.errorMessage || path)}`,
			);
	...
	async publishPreview(input: ValidatedPreviewInput): Promise<PreviewState> {
		const session = await this.requireSession();
		if (!session.getLink)
			throw new Error("当前 AgentBay 沙盒不支持端口预览链接");
		...
		if (!start.success || !/^\d+$/u.test(pid)) {
			throw new Error(
				`无法启动静态预览；目录须包含 index.html、无符号链接/密钥文件且不超过 20 MiB：${safeMessage(start.errorMessage || start.output)}`,
			);
		}
		...
		if (!health.success || !health.output.includes("PREVIEW_HEALTH_OK")) {
			...
			throw new Error(
				`预览服务健康检查失败：${safeMessage(health.errorMessage || health.output)}`,
			);
		}
		...
		if (!directUrl) {
			try {
				...
				throw new Error(
					`端口预览不可用（${directError}）；浏览器回退也失败：${safeMessage(error)}`,
				);
			}
		}

		const previewUrl = directUrl ?? takeoverUrl;
		if (!previewUrl) throw new Error("预览链接创建失败");
		this.previewPid = pid;
		this.preview = {
			...input,
			url: previewUrl,
		...
		this.previewTimer = setTimeout(
			() => void this.destroy("previewExpired"),
			this.previewTtlMs,
		);
	...
	private async openBrowser(url: string): Promise<string> {
		const session = await this.requireSession();
		if (!session.browser || !session.info)
			throw new Error("当前沙盒镜像不支持浏览器人工接管");
		const initialized = await session.browser.initializeAsync({
			callForUser: true,
			defaultNavigateUrl: url,
		});
		if (!initialized) throw new Error("AgentBay 浏览器初始化失败");
		this.browserInitialized = true;
		const info = await session.info();
		if (!info.success)
			throw new Error(info.errorMessage || "AgentBay 浏览器接管链接创建失败");
	...
	async stopPreview(): Promise<void> {
		...
		if (this.browserInitialized && session?.browser) {
			try {
				await session.browser.destroy();
			} catch {
				// Browser cleanup is best effort; sandbox deletion handles final cleanup.
			}
		}
		...
		if (session && pid) {
			try {
				...
				// Preview process cleanup is best effort.
			}
		}
	...
	async destroy(
		reason: "idle" | "closed" | "reset" | "previewExpired" = "closed",
	): Promise<void> {
		if (this.destroyPromise) return this.destroyPromise;
		this.destroyPromise = (async () => {
			if (this.idleTimer) clearTimeout(this.idleTimer);
			if (this.previewTimer) clearTimeout(this.previewTimer);
			this.setStatus("destroying", destroyingDetail(reason));
			if (this.startPromise) {
				try {
					await this.startPromise;
				} catch {
					// A failed start is reported by start(); destruction still proceeds.
				}
			}
			this.browserInitialized = false;
			this.preview = undefined;
			...
			if (session) {
				try {
					...
					if (!result.success)
						throw new Error(result.errorMessage || "AgentBay 删除失败");
				} catch (error) {
					this.setStatus("error", `沙盒清理失败：${safeMessage(error)}`);
					return;
				}
			}
			this.setStatus("destroyed", destroyedDetail(reason));
		})();
