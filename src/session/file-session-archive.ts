...
async function readEntries(
	...
	try {
		...
		if (info.size > MAX_ARCHIVE_BYTES)
			throw new Error("外部会话文件超过 100 MiB 限制");
		text = await readFile(path, "utf8");
	} catch (error) {
		if (missingAsNull && (error as NodeJS.ErrnoException).code === "ENOENT")
			return null;
		throw error;
	}
...
function sessionIdFrom(entries: SessionEntry[], path: string): string {
	for (const entry of entries) {
		const candidate =
			typeof entry.sessionId === "string" ? entry.sessionId : entry.session_id;
...
export class FileSessionArchive implements SessionArchivePort {
	...
	constructor(
		...
		boundFiles = new Map<string, string>(),
	) {
	...
	static async resumeFromId(
		...
		return FileSessionArchive.resumeFrom(
			join(resolve(root), `${sessionId}.jsonl`),
		);
	}

	static async resumeFrom(
		reference: string,
	): Promise<{ archive: FileSessionArchive; sessionId: string; path: string }> {
		...
		const archive = new FileSessionArchive(
			dirname(path),
			new Map([[sessionId, path]]),
		);
	...
	pathFor(sessionId: string, subpath?: string): string {
		...
		if (!subpath)
			return (
				this.boundFiles.get(sessionId) || join(this.root, `${sessionId}.jsonl`)
			);
	...
	async append(key: SessionKey, entries: SessionEntry[]): Promise<void> {
		...
		await this.serial(path, async () => {
			...
			const additions = entries.filter(
				(entry) =>
					!entry.uuid ||
					(!uuids.has(entry.uuid) && Boolean(uuids.add(entry.uuid))),
			);
	...
	async confirm(sessionId: string): Promise<string> {
		...
		if (!entries?.length)
			throw new Error("Claude 会话尚未写入外部存储，已取消销毁");
		return path;
	}

	private async serial(
		path: string,
		...
		const current = (async () => {
			try {
				...
				// A previous append must not block the next queued write.
			}
			await operation();
		})();
