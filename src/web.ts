...
import type { AccountUsageSnapshot, ShutdownAllResult } from "./account/application.js";
...
import { createAccountUsageService, createAgentChatSession, createAliyunBillExporter, rescueAgentChatSessionById } from "./composition.js";
...
import { createWebRequestHandler, freshWebConversation, type WebMessage, type WebRuntimePort } from "./web-http.js";
...
class SharedWebRuntime implements WebRuntimePort {
  ...
  private messages: WebMessage[] = [{ id: 0, role: "system", text: "这是一个可信 LAN 共享会话；前端任务会生成临时预览，可按需人工接管浏览器。" }];
  ...
  private note(text: string): void {
    this.messages = [...this.messages.slice(-49), { id: this.nextId++, role: "system", text }];
  }

  private step(text: string): void {
    if (this.processSteps.at(-1)?.text === text) return;
    this.processSteps = [...this.processSteps.slice(-7), { id: this.nextProcessId++, text }];
  }

  private async refreshAccount(includeMetrics = true, detectExternalShutdown = false): Promise<void> {
    try {
      this.accountUsage = await this.account.snapshot({ includeMetrics });
      this.accountError = "";
      const currentId = this.session.sandboxSessionId;
      if (detectExternalShutdown && currentId && !this.accountUsage.sessions.some((session) => session.sessionId === currentId)) {
        await this.session.close("reset");
        this.session = this.makeSession();
        this.sandboxState = "destroyed";
        this.status = "当前沙盒已从其他入口关闭；下一次提问或 /new 将创建新沙盒";
  ...
  async start(): Promise<void> {
    ...
    this.busy = false;
    this.timer = setInterval(() => { if (!this.busy) void this.refreshAccount(false, true); }, 30_000);
    this.timer.unref();
  }
  ...
  snapshot(): { [key: string]: unknown; busy: boolean } {
    return {
      ...
      billStatus: formatBillStatusLine(this.billSpend, this.billLoading, this.billError),
      busy: this.busy,
  ...
  async resume(sessionId: string): Promise<{ sessionId: string }> {
    ...
    this.step("正在校验受限归档目录中的会话");
    let rescued: Awaited<ReturnType<typeof rescueAgentChatSessionById>> | undefined;
    try {
      rescued = await rescueAgentChatSessionById(sessionId, this.onSandboxStatus);
      const current = this.session;
      await archiveAndDestroy({ archive: () => current.archive(), destroy: () => current.close("reset") });
      this.session = rescued.session;
      this.processSteps = [];
      this.note(`已恢复 Claude 会话 ${rescued.sessionId}；正在创建新的独立沙盒`);
      await this.session.start();
      await this.refreshAccount();
      return { sessionId: rescued.sessionId };
    } catch (error) {
      if (rescued && this.session !== rescued.session) await rescued.session.close("reset");
      ...
      throw Object.assign(new Error(notFound ? "找不到该会话归档" : safeMessage(error)), { statusCode: notFound ? 404 : 500 });
    } finally {
      this.busy = false;
    }
  }
  ...
  async exitCurrent(): Promise<{ archived: boolean; sessionId?: string }> {
    ...
    this.step("正在安全退出当前共享会话");
    const current = this.session;
    try {
      const result = await archiveAndDestroy({ archive: () => current.archive(), destroy: () => current.close() });
      this.session = this.makeSession();
      this.sandboxState = "destroyed";
      this.status = "当前 Web 会话已安全退出；服务保持在线";
      this.step(this.status);
      const sessionId = sessionIdFromArchive(result.archivePath);
      this.note(sessionId ? `当前会话已归档并退出；恢复 ID：${sessionId}` : "当前会话已退出（尚无聊天记录）");
      await this.refreshAccount(false);
      return { archived: Boolean(result.archivePath), ...(sessionId ? { sessionId } : {}) };
    } catch (error) {
  ...
  async shutdownAll(): Promise<ShutdownAllResult> {
    ...
    try {
      ...
      this.note(`${this.status}${archivedId ? `；共享会话恢复 ID：${archivedId}` : ""}。发送新问题或 /new 时才会创建新沙盒。`);
      await this.refreshAccount(false);
      return result;
    } catch (error) {
  ...
  async submit(raw: string): Promise<void> {
    ...
    try {
      if (command.type === "new") {
        ...
        const archived = await archiveAndDestroy({ archive: () => previous.archive(), destroy: () => previous.close("reset") });
        this.session = this.makeSession();
        this.processSteps = [];
        const archivedId = sessionIdFromArchive(archived.archivePath);
        this.messages = freshWebConversation(this.nextId++, archivedId ? `旧会话已保存；恢复 ID：${archivedId}` : "正在创建全新共享会话");
        await this.session.start();
        await this.refreshAccount();
        return;
      }
      ...
      if (command.type === "preview") {
        const preview = await this.session.publishPreview(command.directory, command.port);
        this.note(preview.takeoverUrl
      ...
      if (command.type === "help") {
        this.note("全部命令：/help · /new · /sandboxes (/sessions) · /bill · /preview <目录> [30100–30199] · /takeover · /stop-preview · /resume <session-id> (/rescue) · /exit (/quit) · /shutdown-all SHUTDOWN ALL");
        return;
      }
      if (command.type === "resume") {
        this.note("Web 恢复只接受 Claude sessionId；请从命令面板填写，不能传入宿主机路径");
        return;
      }
      if (command.type === "exit") {
        this.note("请使用命令面板的 /exit；浏览器会在安全归档后尝试关闭当前窗口");
        return;
      }
      ...
      this.messages = [...this.messages.slice(-48), { id: this.nextId++, role: "user", text: command.text }, { id: assistantId, role: "assistant", text: "" }];
      this.processSteps = [{ id: this.nextProcessId++, text: "请求已进入 Agent 执行队列" }];
      this.status = "Agent 正在执行…";
      for await (const event of this.session.send(command.text)) {
        if (event.type === "text") {
          this.messages = this.messages.map((message) => message.id === assistantId ? { ...message, text: message.text + event.text } : message);
        } else {
          this.status = event.text;
          this.step(event.text);
        }
      }
    } catch (error) {
  ...
  async close(): Promise<void> {
    ...
    await archiveAndDestroy({ archive: () => this.session.archive(), destroy: () => this.session.close() });
  }
}

function lanAddress(): string {
  const entries = Object.entries(networkInterfaces());
  const ordered = [...entries.filter(([name]) => name === "en0"), ...entries.filter(([name]) => name !== "en0")];
...
async function main(): Promise<void> {
  ...
  if (!Number.isInteger(port) || port < 1 || port > 65_535) throw new Error("AGENTBAY_WEB_PORT 无效");
