...
import { type AgentChatSession, EFFORT, MODEL, REASONING } from "./agent.js";
...
export function App(): React.JSX.Element {
	...
	const accountServiceRef = useRef<
		ReturnType<typeof createAccountUsageService> | undefined
	>(undefined);
	const billExporterRef = useRef<
		ReturnType<typeof createAliyunBillExporter> | undefined
	>(undefined);
	const accountRefreshRef = useRef<Promise<AccountUsageSnapshot> | undefined>(
		undefined,
	);
	const billRefreshRef = useRef<Promise<AgentBaySpendSnapshot> | undefined>(
		undefined,
	);
	...
	const [messages, setMessages] = useState<Message[]>([
		{
			id: 0,
			role: "system",
			text: "输入问题开始；前端任务会发布临时预览；/takeover 人工接管浏览器；/exit 保存并销毁。",
		},
	]);
	...
	const [billSpend, setBillSpend] = useState<AgentBaySpendSnapshot | undefined>(
		undefined,
	);
	...
	if (!accountServiceRef.current)
		accountServiceRef.current = createAccountUsageService();
	if (!billExporterRef.current)
		billExporterRef.current = createAliyunBillExporter();
	...
	const refreshAccountUsage = useCallback(
		async (
			...
			const pending = accountServiceRef
				.current!.snapshot({ includeMetrics })
				.then(async (snapshot) => {
					...
					if (
						...
						setMessages((items) => [
							...items,
							{
								id: nextId.current++,
								role: "system",
								text: "检测到当前沙盒已被 LAN Web 或本机命令关闭。",
							},
						]);
					}
					return snapshot;
				...
				.finally(() => {
					if (accountRefreshRef.current === pending)
						accountRefreshRef.current = undefined;
				});
	...
	const refreshBillSpend =
		useCallback(async (): Promise<AgentBaySpendSnapshot> => {
			if (billRefreshRef.current) return billRefreshRef.current;
			setBillLoading(true);
			const pending = billExporterRef
				.current!.export()
				.then((snapshot) => {
				...
				.finally(() => {
					...
					if (billRefreshRef.current === pending)
						billRefreshRef.current = undefined;
				});
			billRefreshRef.current = pending;
			return pending;
		}, []);

	const appendTrace = useCallback((messageId: number, text: string) => {
		setMessages((items) =>
			items.map((item) => {
				if (
					item.id !== messageId ||
					!item.trace ||
					item.trace.steps.at(-1)?.text === text
				)
					return item;
				return {
					...item,
					trace: {
						...item.trace,
						steps: [
							...item.trace.steps.slice(-6),
							{ id: nextProcessId.current++, text },
						],
					},
				};
			}),
	...
	const finishTrace = useCallback(
		(
			...
			setMessages((items) =>
				items.map((item) =>
					item.id === messageId && item.trace
						? { ...item, trace: { ...item.trace, status, durationMs } }
						: item,
				),
			);
		},
	...
	const makeSession = useCallback(
		() => createAgentChatSession(onSandboxStatus),
		[onSandboxStatus],
	);
	...
	useEffect(() => {
		...
		const refreshTimer = setInterval(
			() => void refreshAccountUsage(false, true).catch(() => undefined),
			30_000,
		);
		refreshTimer.unref();
	...
	useInput((value, key) => {
		...
		if (key.ctrl && value === "t") setTraceExpanded((expanded) => !expanded);
	});
	...
	const submit = useCallback(
		async (raw: string) => {
			...
			if (command.type === "error") {
				setMessages((items) => [
					...items,
					{ id: nextId.current++, role: "system", text: command.message },
				]);
				return;
			}
			if (command.type === "new") {
			...
			if (command.type === "resume") {
				setBusy(true);
				let rescued:
					| Awaited<ReturnType<typeof rescueAgentChatSession>>
					| undefined;
				try {
					...
					setMessages([
						{
							id: nextId.current++,
							role: "system",
							text: `已从 ${rescued.path} 救援 Claude 会话；正在创建新的 AgentBay 沙盒。`,
						},
					]);
					await start();
				} catch (error) {
				...
				try {
					...
					setMessages((items) => [
						...items,
						{
							id: nextId.current++,
							role: "system",
							text: `远端沙盒已刷新：${snapshot.running}/${snapshot.parallelLimit} 并行，参考估算 ¥${snapshot.estimatedCnyPerHour.toFixed(2)}–${snapshot.estimatedCnyPerHourHigh.toFixed(2)}/小时。`,
						},
					]);
				} catch (error) {
				...
				try {
					...
					setMessages((items) => [
						...items,
						{
							id: nextId.current++,
							role: "system",
							text: `官方 BSS 按量消费已刷新：${formatBillStatusLine(bill)}；明细 ${bill.path}`,
						},
					]);
				} catch (error) {
			...
			if (command.type === "preview") {
				setBusy(true);
				try {
					const published = await sessionRef.current!.publishPreview(
						command.directory,
						command.port,
					);
					...
					setMessages((items) => [
						...items,
						{
							id: nextId.current++,
							role: "system",
							text: `${published.takeoverUrl ? "可人工接管的浏览器预览" : "临时应用"}已发布：${published.url}\n到期：${published.expiresAt}`,
						},
					]);
				...
				try {
					...
					setMessages((items) => [
						...items,
						{
							id: nextId.current++,
							role: "system",
							text: `浏览器已打开预览；人工接管：${current.takeoverUrl}`,
						},
					]);
					await refreshAccountUsage(false);
				} catch (error) {
				...
				try {
					...
					setMessages((items) => [
						...items,
						{
							id: nextId.current++,
							role: "system",
							text: `${summary}${archivePath ? `；会话已保存：${archivePath}` : ""}。下一次提问或 /new 才会创建新沙盒。`,
						},
					]);
					try {
						const snapshot = await accountServiceRef.current!.snapshot({
			...
			if (command.type === "help") {
				setMessages((items) => [
					...items,
					{
						id: nextId.current++,
						role: "system",
						text: "/preview <目录> 发布临时前端 · /takeover 人工接管浏览器 · /stop-preview 停止 · /sandboxes 远端并行 · /bill 消费 · /shutdown-all SHUTDOWN ALL 全停 · /new 新建 · /resume <JSONL> 恢复 · /exit 保存销毁",
					},
				]);
				return;
			}
			...
			setMessages((items) => [
				...
				{ id: nextId.current++, role: "user", text: prompt },
				{
					...
					trace: {
						steps: [
							{
								id: nextProcessId.current++,
								text: "请求已进入 Agent 执行队列",
							},
						],
						status: "running",
					},
			...
			try {
				for await (const event of sessionRef.current!.send(prompt)) {
					if (event.type === "text") {
						...
						appendTrace(assistantId, event.text);
					}
				}
				finishTrace(assistantId, "complete", Date.now() - startedAt);
			} catch (error) {
				...
				setStatus(text);
				appendTrace(assistantId, text);
				finishTrace(assistantId, "error", Date.now() - startedAt);
			} finally {
	...
	return (
		<Box flexDirection="column" paddingX={1}>
			...
			<Box
				...
				borderStyle="single"
				borderColor={
					accountUsage && accountUsage.running >= accountUsage.parallelLimit
						? "red"
						: "blue"
				}
				paddingX={1}
				...
				<Box justifyContent="space-between">
					...
					{accountUsage && (
						<Text
							bold
							color={
								accountUsage.running >= accountUsage.parallelLimit
									? "red"
									: "cyan"
							}
						>
							{accountUsage.running}/{accountUsage.parallelLimit} RUNNING ·{" "}
							{accountUsage.utilizationPercent}%
						</Text>
					)}
				</Box>
				{accountUsage ? (
					<>
						{accountUsage.sessions.map((session) => (
							<Text key={session.sessionId}>
								...
								<Text dimColor> · {session.imageId || "unknown"}</Text>
								{session.estimatedCnyPerHour === undefined ? (
									<Text color="yellow"> · 指标暂不可用</Text>
								) : (
									<Text dimColor>
										{" "}
										· {session.cpuCount}C/{session.memoryGiB}GiB · ¥
										{session.estimatedCnyPerHour.toFixed(2)}–
										{session.estimatedCnyPerHourHigh!.toFixed(2)}/h
									</Text>
								)}
							</Text>
						))}
						{!accountUsage.sessions.length && (
							<Text dimColor>○ 当前账户没有 RUNNING 沙盒</Text>
						)}
						<Text dimColor>
							Basic · 估算覆盖 {accountUsage.estimatedSessionCount}/
							{accountUsage.running}；官方已入账消费见底部状态行
						</Text>
					</>
				) : (
			...
			{preview && (
				...
				>
					<Text bold color="green">
						▣ TEMPORARY APP PREVIEW
					</Text>
					<Text>
						<Text color="cyan">
							{preview.takeoverUrl === preview.url ? "BROWSER  " : "APP      "}
						</Text>
						{preview.url}
					</Text>
					{preview.takeoverUrl && preview.takeoverUrl !== preview.url && (
						<Text>
							<Text color="magenta">TAKEOVER </Text>
							{preview.takeoverUrl}
						</Text>
					)}
					<Text dimColor>
						{preview.directory} · :{preview.port} · 到期{" "}
				...
				{messages.slice(-10).map((message) => {
					...
					return (
						<Box
							key={message.id}
							flexDirection="column"
							marginBottom={message.role === "system" ? 0 : 1}
						>
							<Text>
								<Text bold color={color}>
									{label.padEnd(5)}{" "}
								</Text>
								<Text>{message.text || (busy ? "…" : "")}</Text>
							</Text>
							{trace && (
								<Box
									flexDirection="column"
									...
									marginTop={1}
								>
									<Text bold color={traceColor}>
										{trace.status === "error"
											? "×"
											: trace.status === "running"
												? "◆"
												: "✓"}{" "}
										EXECUTION TRACE · {traceSummary}
										{duration} {traceExpanded ? "⌃" : "⌄"}
									</Text>
									{traceExpanded &&
										trace.steps.map((step, index) => {
											...
											return (
												<Text
													key={step.id}
													color={failed ? "red" : active ? "yellow" : "green"}
												>
													{failed ? "×" : active ? "◆" : "✓"} {step.text}
												</Text>
											);
										})}
								</Box>
			...
			<Box
				...
				marginTop={1}
			>
				<Text bold color={busy ? "yellow" : "cyan"}>
					{busy ? "WAIT " : "ASK  "}
				</Text>
				<TextInput
					value={input}
					...
					focus={!busy}
					placeholder={busy ? "Agent 正在执行…" : "输入问题或 /help"}
				/>
			</Box>
			<Box paddingX={1} justifyContent="space-between">
				<Text color={billError ? "red" : billSpend ? "green" : "yellow"}>
					￥ {formatBillStatusLine(billSpend, billLoading, billError)}
				</Text>
				<Text dimColor>Esc 中止 · Ctrl+T 展开/收起轨迹 · Ctrl+C 安全退出</Text>
			</Box>
			<Text dimColor>
				{" "}
				/preview &lt;目录&gt; 发布 · /takeover 接管 · /stop-preview 停止 ·
				/sandboxes 刷新 · /bill 消费 · /new · /resume
			</Text>
		</Box>
	);
}
...
async function smoke(): Promise<void> {
	const session = createAgentChatSession(({ state, detail }) =>
		console.log(`[sandbox:${state}] ${detail}`),
	);
	let answer = "";
	try {
		await session.start();
		for await (const event of session.send(
			"必须调用 AgentBay bash 执行 printf AGENTBAY_HELLO，然后只回复命令的真实输出。",
		)) {
			if (event.type === "text") answer += event.text;
			else console.log(`[sdk] ${event.text}`);
		}
		if (!answer.includes("AGENTBAY_HELLO"))
			throw new Error(`Smoke 未得到远程输出：${answer}`);
...
async function rescueSmoke(): Promise<void> {
	...
	const first = createAgentChatSession(({ state, detail }) =>
		console.log(`[first:${state}] ${detail}`),
	);
	let archivePath: string | undefined;
	try {
		await first.start();
		for await (const event of first.send(
			`请记住救援口令 ${rescueCode}，只回复 STORED。`,
		)) {
	...
	const rescued = await rescueAgentChatSession(
		archivePath,
		({ state, detail }) => console.log(`[rescued:${state}] ${detail}`),
	);
	let answer = "";
	try {
		await rescued.session.start();
		for await (const event of rescued.session.send(
			"刚才让我记住的救援口令是什么？只回复口令。",
		)) {
			if (event.type === "text") answer += event.text;
			else console.log(`[rescued-sdk] ${event.text}`);
		}
		if (!answer.includes(rescueCode))
			throw new Error(`恢复后未找回上下文：${answer}`);
...
async function accountSmoke(): Promise<void> {
	...
	console.log(
		`ACCOUNT_PASS running=${snapshot.running}/${snapshot.parallelLimit} estimated_cny_per_hour=${snapshot.estimatedCnyPerHour.toFixed(2)}-${snapshot.estimatedCnyPerHourHigh.toFixed(2)} metrics=${snapshot.estimatedSessionCount}/${snapshot.running}`,
	);
	const bill = await createAliyunBillExporter().export();
	console.log(
		`BILL_SPEND_PASS cycle=${bill.cycle} gross_cny=${bill.totalPretaxGrossAmount.toFixed(2)} charged_cny=${bill.totalPretaxAmount.toFixed(2)} path=${bill.path}`,
	);
}
...
if (process.argv.includes("--smoke")) {
	...
	if (!process.stdin.isTTY || !process.stdout.isTTY)
		throw new Error(
			"请在 TTY/Herdr 中运行 Ink TUI；自动验证请用 npm run smoke。",
		);
