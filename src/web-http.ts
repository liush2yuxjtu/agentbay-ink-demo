...
import { SHUTDOWN_ALL_CONFIRMATION, type ShutdownAllResult } from "./account/application.js";
import { safeMessage } from "./sandbox.js";
import { isClaudeSessionId } from "./session/file-session-archive.js";

export type WebMessage = { id: number; role: "user" | "assistant" | "system"; text: string };
...
function send(response: ServerResponse, status: number, body: string, type = "text/plain; charset=utf-8", nonce?: string): void {
  response.writeHead(status, { ...securityHeaders(nonce), "content-type": type });
  response.end(body);
}
...
async function readJsonBody(request: IncomingMessage): Promise<unknown> {
  ...
  for await (const chunk of request) {
    ...
    size += buffer.length;
    if (size > 16_384) throw Object.assign(new Error("请求内容过大"), { statusCode: 413 });
    chunks.push(buffer);
  }
  try {
...
export function createWebRequestHandler(runtime: WebRuntimePort, htmlTemplate: string) {
  return async (request: IncomingMessage, response: ServerResponse): Promise<void> => {
    try {
      const url = new URL(request.url || "/", "http://lan.local");
      if (request.method === "GET" && url.pathname === "/health") return json(response, 200, { ok: true });
      if (request.method === "POST" && isCrossSite(request)) return json(response, 403, { error: "拒绝跨站写请求" });
      if (request.method === "POST" && !request.headers["content-type"]?.startsWith("application/json")) {
        return json(response, 415, { error: "写请求必须使用 application/json" });
      }

      if (request.method === "GET" && url.pathname === "/") {
        const nonce = randomBytes(18).toString("base64url");
        return send(response, 200, htmlTemplate.replaceAll("__CSP_NONCE__", nonce), "text/html; charset=utf-8", nonce);
      }
      ...
      return json(response, status, { error: status === 500 ? "服务器内部错误" : safeMessage(error) });
    }
  };
}
