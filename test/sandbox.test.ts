...
function fakeSession(onDelete: () => void) {
  return {
    ...
    command: {
      executeCommand: async (command: string) => ({ success: true, output: `remote:${command}`, exitCode: 0 }),
    },
    fileSystem: {
      readFile: async (path: string) => ({ success: true, content: `file:${path}` }),
      writeFile: async () => ({ success: true }),
    },
    delete: async () => { onDelete(); return { success: true }; },
  };
}

test("错误信息会隐藏凭据、租户和请求标识", () => {
  const message = safeMessage(new Error("tenantId: tenant-fixture-123 request id: 123e4567-e89b-42d3-a456-426614174000 token=secret-value"));
...
test("一个会话只创建一个沙盒，并在空闲后只销毁一次", async () => {
  ...
  const sandbox = new EphemeralSandbox({
    idleMs: 30,
    createSandbox: async () => { created++; return fakeSession(() => deleted++); },
  });
...
test("创建尚未完成时退出也会清理刚创建的沙盒", async () => {
  ...
  let deleted = 0;
  const pending = new Promise<ReturnType<typeof fakeSession>>((resolve) => { release = resolve; });
