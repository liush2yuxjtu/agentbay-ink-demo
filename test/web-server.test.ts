...
test("LAN Web 无分享令牌即可访问，并保护同源写请求", async () => {
	...
	const runtime = {
		...
		shutdownAll: async () => {
			shutdowns++;
			return { attempted: 2, deleted: 2, failures: [] };
		},
	...
	const html =
		'<!doctype html><html><body>agentBayDemo · /Users/liushiyuwin/projects/agentBayDemo · 019fc543-1e19-7728-a9f1-7a3aa077c0ce<script nonce="__CSP_NONCE__"></script></body></html>';
	...
	try {
		...
		const abort = await fetch(`${origin}/api/abort`, {
			method: "POST",
			headers: jsonHeaders,
			body: "{}",
		});
		...
		const shutdown = await fetch(`${origin}/api/shutdown-all`, {
			method: "POST",
			headers: jsonHeaders,
			body: JSON.stringify({ confirmation: "SHUTDOWN ALL" }),
		});
		...
		const exit = await fetch(`${origin}/api/exit`, {
			method: "POST",
			headers: jsonHeaders,
			body: "{}",
		});
		assert.equal(exit.status, 200);
		assert.deepEqual(await exit.json(), {
			archived: true,
			sessionId: "123e4567-e89b-42d3-a456-426614174000",
		});
		...
		const busyExit = await fetch(`${origin}/api/exit`, {
			method: "POST",
			headers: jsonHeaders,
			body: "{}",
		});
...
test("Web 页面展示完整 slash 命令、临时预览、人工接管、新手指引与关闭回退", async () => {
	...
	assert.match(html, /renderAgentTrace/);
	assert.doesNotMatch(html, /id="trace"/);
	assert.doesNotMatch(html, /processSteps/);
	assert.match(html, /data-testid="agent-root"/);
	assert.match(html, /data-agent-state="running"/);
